...
import "../ast/ast" for Program, BlockStatement, LetStatement, ReturnStatement, ExpressionStatement, BinaryExpression, UnaryExpression, Identifier, BooleanLiteral, EmptyLiteral, IntegerLiteral, StringLiteral, IfExpression, FunctionExpression, CallExpression, IndexExpression, ArrayLiteral, HashLiteral, HashMember
...
class Parser is PrattParser {
	construct new(lexer) {
		...
		prefixTable[Tok["IF"].value] = PrattEntry.new(
			...
			Fn.new {|o| this.parseIfExpression()})
		...
		prefixTable[Tok["INTEGER_LIT"].value] = PrattEntry.new(
			Tok["INTEGER_LIT"], Prec["PRE"], false,
			Fn.new {|o| this.parseIntegerLiteral()})
	...
	parseLetStatement() {
		...
		return LetStatement.new(name, value)
	}
	...
	parseIntegerLiteral() {
		...
		advance()
		return IntegerLiteral.new(value)
	}
	...
	parseIfExpression() {
		...
		if (match(Tok["ELSE"])) {
			...
			return IfExpression.new(cond, thenBlock, elseBlock)
		} else {
			return IfExpression.new(cond, thenBlock, null)
		}
	}
	...
	parseArrayLiteral() {
		...
		//System.print("found exprs " + exprs.count.toString)
		if (!match(Tok["RBRACK"])) {
			return null
		}
	...
	parseKeyLiteral(opToken, left) {
		...
		return HashMember.new(left, right)
	}
